...
public class WebServiceClient {
    ...
    private int findArticleById(BufferedReader in) throws IOException {
        ...
        List<Article> Articles = this.ArticleService.getArticleWebServicePort().getArticles(
                new ArrayList<ArticleFieldValue>() {{
                    add(new ArticleFieldValue(Field.ID, id));
                }});

        if (Articles.size() == 0) {
            System.err.println("Article with such id was not found");
            return -1;
        } else {
            System.out.println("Article found: " + Articles.get(0));
            return id;
        }
    }
